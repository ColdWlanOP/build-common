...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/ColdWlanOP/build-common build/common
...
git clone -b main --depth 1 https://github.com/ColdWlanOP/ColdWlanOP shangyou
...
if [[ "${GIT_REPOSITORY}" =~ (ColdWlanOP/ColdWlanOP|ColdWlanOP/autobuild) ]]; then
  rm -rf backups
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="ColdWlanOP/ColdWlanOP"
  sudo rm -rf build common.sh
